...
0. **ตรวจสอบ Timezone ปัจจุบัน**
   ...
   ![แสดง Timezone ปัจจุบันใน DevTools](/blog/test-multi-timezone-with-chrome-devtools/1.png)

1. **เปิด Chrome DevTools**

   กด `F12` หรือคลิกขวาเลือก Inspect
   ![เปิด Chrome DevTools](/blog/test-multi-timezone-with-chrome-devtools/2.png)

2. **เปิดเมนู Sensors**

   กดปุ่ม `...` (สามจุดมุมขวาบนของ DevTools)
   เลือก `More tools` > `Sensors`
   ![เปิดเมนู Sensors ใน DevTools](/blog/test-multi-timezone-with-chrome-devtools/3.png)
...
3. **เปลี่ยน Location และ Timezone**
   ...
   ![เปลี่ยน Location และ Timezone ใน DevTools](/blog/test-multi-timezone-with-chrome-devtools/4.png)

4. **Refresh หน้าเว็บ**

   กด Refresh เพื่อให้เว็บโหลดข้อมูลใหม่ จะเห็นว่าเวลาบนเว็บเปลี่ยนเป็นเวลาของ Timezone ที่เลือก
   ![Refresh หน้าเว็บหลังเปลี่ยน Timezone](/blog/test-multi-timezone-with-chrome-devtools/5.png)
...
5. **เปลี่ยน Location อื่น ๆ ได้ด้วย**
   ...
   ![เปลี่ยน Location อื่น ๆ ได้ด้วย](/blog/test-multi-timezone-with-chrome-devtools/6.png)

6. **กลับมาใช้ Timezone เดิม**

   หากต้องการกลับมาใช้ Timezone ของเครื่อง ให้เปลี่ยน Location กลับเป็น `No override` เท่านั้น
   ![กลับมาใช้ Timezone เดิม](/blog/test-multi-timezone-with-chrome-devtools/7.png)
